...
data = np.load(opj(idir, f'data_draws_test.npy'))
print(data.shape)
nsim = data.shape[0]
for idx in range(0,nsim):
    ax.plot(data[idx], color='C0', alpha=0.5, lw=0.5, label=f'test_data' if idx == 0 else None)
...
fig.savefig(opj(imgdir, 'test_data_draws'))
...
ax.legend(frameon=False)
fig.savefig(opj(imgdir, 'test_data_draws_unnorm'))
...
nsim = data.shape[0]
for idx in range(0,nsim):
    ax.plot(data[idx] - data_true, color='C0', alpha=0.5, lw=0.5, label=f'test_data' if idx == 0 else None)
