...
if __name__ == '__main__':
    ...
    if comm.rank == 0:
        ...
        fig.savefig(opj(args.odir, 'img', 'optimization_history'))
        plt.close(fig)

        ax = oplt.plot_param_importances(study)
        fig = get_figure_from_ax(ax)
        fig.set_constrained_layout(True)
        fig.savefig(opj(args.odir, 'img', 'param_importance'))
        plt.close(fig)

        ax = oplt.plot_slice(study)
        fig = get_figure_from_ax(ax)
        fig.set_constrained_layout(True)
        fig.savefig(opj(args.odir, 'img', 'slice'))
        plt.close(fig)

        ax = oplt.plot_contour(study)
        fig = get_figure_from_ax(ax)
        fig.set_constrained_layout(True)
        fig.savefig(opj(args.odir, 'img', 'contour'))
        plt.close(fig)
