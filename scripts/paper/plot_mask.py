...
mask_enmap = reproject.healpix2map(mask, shape=shape, wcs=wcs, method='spline', order=1)
beta_enmap = reproject.healpix2map(beta_d, shape=shape, wcs=wcs, method='spline', order=1)

mask_enmap[mask_enmap < 0.001] = 0
mask_enmap[mask_enmap > 0.05] = 0
mask_enmap[mask_enmap != 0] = 1
...
smooth_mask = gaussian_filter(mask_enmap.astype(float), sigma=3)

ax.imshow(np.ones_like(smooth_mask),  # white image
...
fig.savefig(opj(imgdir, 'mask.pdf'))
plt.close(fig)
