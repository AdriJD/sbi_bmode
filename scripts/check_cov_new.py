'''
Update after adding sync, and coadding of equivalent cross spectra.
'''
import os
import yaml
import pickle
...
def get_prior(params_dict):
    ...
    return prior_combined, param_names

def main(odir, config, specdir, seed, coadd_equiv_crosses=True):

    data_dict, fixed_params_dict, params_dict = script_utils.parse_config(config)
    prior_combined, param_names = get_prior(params_dict)
    ...
    if comm.rank == 0:
        ...
        # Reshape analytic cov.
        tri_indices = sim_utils.get_tri_indices(cmb_simulator.nsplit, cmb_simulator.nfreq)
        
        if coadd_equiv_crosses:
            n_unique = coadd_mat.shape[0]
            cov_ext = np.ones((n_unique, n_unique, cov.shape[-1], cov.shape[-1]))
        else:    
        ...
        if coadd_equiv_crosses:
            diff = likelihood_utils.get_diff(
                sims_mean.reshape(n_unique, -1), model, tri_indices, coadd_matrix=coadd_mat)        
        else:
            diff = likelihood_utils.get_diff(
                sims_mean.reshape(tri_indices.shape[0], -1), model, tri_indices, coadd_matrix=coadd_mat)
        ...
        for idx in range(nsamp):
            if coadd_equiv_crosses:
                diff_samples[idx] = likelihood_utils.get_diff(
                    sims[idx].reshape(n_unique, -1),
                    model, tri_indices, coadd_matrix=coadd_mat).reshape(-1)            
            else:
                diff_samples[idx] = likelihood_utils.get_diff(
                    sims[idx].reshape(tri_indices.shape[0], -1),
                    model, tri_indices, coadd_matrix=coadd_mat).reshape(-1)
...
if __name__ == '__main__':
    ...
    main(args.odir, config, args.specdir, args.seed, coadd_equiv_crosses=not args.no_coadd_equiv_crosses)
