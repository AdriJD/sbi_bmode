...
class CMBSimulator:
    ...
    def __init__(
        ...
        use_dbeta_map=False,
        use_sync_map=False,
        use_dbeta_sync_map=False,
        deproj_dust=False,
        deproj_dbeta=False,
        deproj_sync=False,
        deproj_dbeta_sync=False,
        fiducial_beta=None,
        fiducial_T_dust=None,
        ...
        freq_strings_ordered = np.asarray(freq_strings)[
            np.argsort(np.asarray(beam_fwhms))
        ][::-1]
        ...
        if pyilcdir:
            ...
            if self.use_dust_map:
                self.ncomp += 1
            if self.use_dbeta_map:
                self.ncomp += 1
            if self.use_sync_map:
                self.ncomp += 1
            if self.use_dbeta_sync_map:
                self.ncomp += 1
        ...
        if self.norm_params and pyilcdir is None:
            # UPDATE WITH SYNC.
            self.norm_model = np.asarray(
                self.get_signal_spectra(
                    norm_params["r_tensor"],
                    norm_params["A_lens"],
                    norm_params["A_d_BB"],
                    norm_params["alpha_d_BB"],
                    norm_params["beta_dust"],
                )
            )
        ...
        if self.score_params and pyilcdir is None:
            ...
            self.score_model = np.asarray(
                self.get_signal_spectra(
                    score_params["r_tensor"],
                    score_params["A_lens"],
                    score_params["A_d_BB"],
                    score_params["alpha_d_BB"],
                    score_params["beta_dust"],
                )
            )
            noise_spectra = self.get_noise_spectra()
    ...
    def get_signal_spectra(
        ...
        if A_s_BB is not None:
            cov_ell = cov_ell.at[:].add(
                spectra_utils.get_sync_spectra(
                    A_s_BB,
                    alpha_s_BB,
                    self.lmax,
                    self.freqs,
        ...
        if rho_ds is not None:
            cov_ell = cov_ell.at[:].add(
                spectra_utils.get_dust_sync_cross_spectra(
                    rho_ds,
                    A_d_BB,
                    alpha_d_BB,
                    A_s_BB,
        ...
        cov_ell = cov_ell.at[:].add(
            spectra_utils.get_combined_cmb_spectrum(
                r_tensor, A_lens, self.cov_scalar_ell, self.cov_tensor_ell
            )[1, 1]
        )
    ...
    def draw_data(
        ...
        if self.pyilcdir:
            ...
            nilc_maps = nilc_utils.get_nilc_maps(
                ...
                self.nside,
                self.fiducial_beta,
                self.fiducial_T_dust,
                self.freq_pivot_dust,
                self.freqs,
                self.beam_fwhms,
                wavelet_type=self.wavelet_type,
                use_dust_map=self.use_dust_map,
                use_dbeta_map=self.use_dbeta_map,
                use_sync_map=self.use_sync_map,
                use_dbeta_sync_map=self.use_dbeta_sync_map,
                deproj_dust=self.deproj_dust,
                deproj_dbeta=self.deproj_dbeta,
                deproj_sync=self.deproj_sync,
                deproj_dbeta_sync=self.deproj_dbeta_sync,
    ...
    def get_unnorm_data(self, data_norm):
        ...
        data = likelihood_utils.get_diff(data, -self.norm_model, tri_indices)
        data = data.reshape(-1)
...
def gen_data(
    ...
    if A_s_BB is not None:
        if (gamma_beta_dust != gamma_beta_sync) and None in (
            gamma_beta_dust,
            gamma_beta_sync,
        ):
            # Raises error only if one of two is None.
            raise ValueError(
                "We only support either both dust and sync gammas or none."
            )
...
def estimate_spectra(imap, minfo, ainfo):
    ...
    for idx, (sidx1, fidx1, sidx2, fidx2) in enumerate(tri_indices):
        out[idx] = ainfo.alm2cl(
            alm[sidx1, fidx1, :, None, :], alm[sidx2, fidx2, None, :, :]
        )[1, 1]
